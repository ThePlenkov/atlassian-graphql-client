...
export interface GenerateFieldTypesOptions {
   ...
   * @default 'gqlb-codegen/field-types' - Use this when consuming as a package
   */
...
export function generateFieldTypes(options: GenerateFieldTypesOptions): string {
  const { schema, schemaTypesImportPath, helpersImportPath = 'gqlb-codegen/field-types' } = options;
